...
/* =====================================================
   🔐 SUPERADMIN : CREATE / UPDATE PAYMENT SETTINGS
===================================================== */
export const upsertSuperAdminPaymentSettings = (req, res) => {
  upload.single("qrImage")(req, res, async (err) => {
    try {
      ...
      if (req.file) {
        const existing = await SuperAdminPaymentSettings.findOne();
...
export const getPublicSuperAdminPayment = async (req, res) => {
  try {
    const payment = await SuperAdminPaymentSettings.findOne({
      isActive: true,
...
export const createOrder = async (req, res) => {
  try {
    ...
    const settings = await SuperAdminPaymentSettings.findOne({
      isActive: true,
